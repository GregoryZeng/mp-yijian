...
export default {
	...
	methods: {
		...
		imageInit(src) {
			...
			uni.getImageInfo({
				...
				success: res => {
					...
					// 临时处理因为 orientation 导致的 width、height 对不上的问题
					if(['left', 'right'].includes(res.orientation)){
						let tmp = res.width;
		...
		init() {
			...
			// console.log('imageRect', imageRect, 'cropperRect', cropperRect)
		...
		// 设置目标图像的大小
		setTarget(){
			console.log('setTarget', this.cropperRect.width)
			// console.log('this.cropperRect','height',this.cropperRect.height,'left',this.cropperRect.left,'top',this.cropperRect.top,'width',this.cropperRect.width)
			// console.log('this.imageRect', 'height',this.imageRect.height,'left',this.imageRect.left,'top',this.imageRect.top,'width',this.imageRect.width)
		...
		async crop() {
			...
			// console.log('dx', dx, 'dy', dy, 'dWidth', this.imageRect.width / scale, 'dHeight', this.imageRect.height / scale)
			// console.log('this.target', 'height', this.target.height, 'width', this.target.width)
